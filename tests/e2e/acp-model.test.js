...
describe('E2E: Claude Code (ACP) Model', () => {
  ...
  beforeAll(async () => {
    ...
    await cursor.launch();
    await cursor.screenshot('2-acp-model-1-launched.png');
  ...
  test('Claude Code (ACP) model works', async () => {
    ...
    await cursor.screenshot('2-acp-model-2-chat-opened.png');

    await cursor.selectModel('Claude Code (ACP)', '2-acp-model-3');
    await cursor.screenshot('2-acp-model-4-model-selected.png');

    // Wait for extension HTTP server to be ready
    await cursor.sleep(5000);

    await cursor.sendChatMessage('Who are you ?');
    await cursor.screenshot('2-acp-model-5-message-sent.png');

    const response = await cursor.waitForChatResponse(60000);
    await cursor.screenshot('2-acp-model-6-response-received.png');

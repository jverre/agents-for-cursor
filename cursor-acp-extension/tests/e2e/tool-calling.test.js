...
describe('E2E: Claude Code (ACP) Tool Calling', () => {
  ...
  beforeAll(async () => {
    ...
    await cursor.launch();
    await cursor.screenshot('tool-call-01-launched.png');
  ...
  test('Tool call bubble appears when running a bash command', async () => {
    ...
    await cursor.screenshot('tool-call-02-chat-opened.png');

    await cursor.selectModel('Claude Code (ACP)', 'tool-call-03');
    await cursor.screenshot('tool-call-04-acp-selected.png');
    ...
    await cursor.screenshot('tool-call-05-message-sent.png');

    // Wait for the response to complete
    await cursor.waitForChatResponse(60000);
    await cursor.screenshot('tool-call-06-response-complete.png');
    ...
    await cursor.screenshot('tool-call-07-final.png');

...
class CursorAutomation {
  ...
  async dismissOnboarding(maxAttempts = 10) {
    console.log('[Test] Checking for onboarding screens...');

    for (let i = 0; i < maxAttempts; i++) {
      await this.sleep(1000);
      await this.screenshot(`onboarding-${i + 1}-check.png`);

      // Try to click onboarding buttons using JavaScript for reliability
      const clicked = await this.mainWindow.evaluate(() => {
        // First try specific onboarding button class
        const onboardingButton = document.querySelector('.onboarding-v2-welcome-button');
        if (onboardingButton) {
          console.log('[Onboarding] Found onboarding-v2-welcome-button, clicking...');
          onboardingButton.scrollIntoView({ behavior: 'instant', block: 'center' });
          onboardingButton.click();
          return 'Onboarding Button';
        }

        // Check for data sharing checkbox and click it if unchecked
        const checkboxLabel = Array.from(document.querySelectorAll('label, div, span')).find(
          el => el.textContent?.includes("I'm fine with Cursor learning")
        );
        if (checkboxLabel) {
          // Find the checkbox input nearby
          const checkbox = checkboxLabel.querySelector('input[type="checkbox"]') ||
                          checkboxLabel.previousElementSibling?.querySelector('input[type="checkbox"]') ||
                          checkboxLabel.parentElement?.querySelector('input[type="checkbox"]') ||
                          document.querySelector('input[type="checkbox"]');
          if (checkbox && !checkbox.checked) {
            console.log('[Onboarding] Found data sharing checkbox, clicking...');
            checkbox.click();
            return 'Data Sharing Checkbox';
          }
          // Also try clicking the label/div itself if no checkbox found
          if (!checkbox) {
            console.log('[Onboarding] Clicking data sharing label...');
            checkboxLabel.click();
            return 'Data Sharing Label';
          }
        }

        // Look for buttons with these texts
        const buttonTexts = ['Continue', 'Skip', 'Get Started', 'Done', 'Next'];

        for (const text of buttonTexts) {
          // Find all potentially clickable elements including divs with cursor-pointer
          const allElements = document.querySelectorAll('button, [role="button"], a, div.cursor-pointer, div[data-click-ready]');
          for (const el of allElements) {
            const elText = el.textContent?.trim();
            if (elText === text || (elText && elText.includes(text) && elText.length < text.length + 10)) {
              console.log(`[Onboarding] Found "${text}" in element, clicking...`);
      ...
      if (clicked) {
        console.log(`[Test] Clicked "${clicked}" (attempt ${i + 1})`);
        await this.sleep(1000);
        await this.screenshot(`onboarding-${i + 1}-after-${clicked.toLowerCase().replace(/ /g, '-')}.png`);
        continue;
      }

      // No more onboarding buttons found
      console.log('[Test] No more onboarding buttons found');
      break;
    }

    await this.screenshot('onboarding-complete.png');
  }
